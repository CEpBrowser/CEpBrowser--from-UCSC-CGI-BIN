...
{
double v;
for (v = 0.0001; v < 1100; v *= 10)
    {
    uglyf("%f %f %d<BR>\n", v, sexp(slog(v)), slog(v));
    }
...
int nullPatSlogProb(DNA *dna, int offset, int patSize)
...
for (i=0; i< endIx; ++i)
    {
    dna = seq+i;
    score = profilePatSlogProb(dna, softMask+i, prof) - nullPatSlogProb(seq, i, profSize);
    if (useLocation)
        score += slogLocProb[i] - slogInvPos;
    if (score > bestScore)
        {
        bestScore = score;
        bestPos = i;
...
{
if (maxOcc == 1)
    return bestMatchInSeq(prof, seqEl, seqSize, slogLocProb, threshold);
else
    {
    struct position *posList, *lastPos;
    int numToTake = maxOcc;
    int i;
    posList = positionsInSeq(prof, seqEl, seqSize, slogLocProb, threshold);
...
for (seqEl = goodSeq; seqEl != NULL; seqEl = seqEl->next)
    {
    newPosList = getPositions(prof, seqEl, seqSize, slogLocProb, 0);
...
for (i = 0; i<4; ++i)
    ...
    for (col = prof->columns; col != NULL; col = col->next)
        {
        fprintf(f, "%4.3f ", sexp(col->slogProb[i]) );
        }
    fprintf(f, "\n");
    }
}
...
void showProfHits(struct profile *prof, struct seqList *seqList, int *retBestScore)
...
for (el = seqList; el != NULL; el = el->next)
    ...
    uppered[seqSize] = 0;
    posList = getPositions(prof, el, seq->size, slogLocProb, 0);
    totalScore = 0;
    for (pos = posList; pos != NULL; pos = pos->next)
        {
        score = pos->score;
    ...
    freePositionList(&posList);
    printf("%5.2f %-*s %s\n", invSlogScale*totalScore, goodSeqNameSize, seq->name, uppered);
    }
freeMem(uppered);
