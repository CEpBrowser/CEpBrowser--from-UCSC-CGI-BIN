...
#ifndef PRIVATE_CGI
boolean CBIsInBrowser = TRUE;		/* is in a comparative browser, default for public cgi */
#else // def PRIVATE_CGI
boolean CBIsInBrowser = FALSE;		/* is not in a comparative browser */
...
#else
		hPrintf("<AREA SHAPE=RECT target=\"_blank\" COORDS=\"%d,%d,%d,%d\" ", x, y, x+width, y+height);
#endif
		hPrintf("HREF=\"%s\"", url);
	...
	* ideoTn parameter is not NULL, it is filled in if the ideogram is created. */
{
	...
	if (doIdeo && !psOutput)
	{
		...
		if(!CBIsInBrowser) {
			...
			hPrintf("<TR><TD><IMG SRC = \"%s\" BORDER=1 WIDTH=%d HEIGHT=%d USEMAP=#%s id='chromNoDrag'>",
				ideoTn->forHtml, ideoWidth, ideoHeight, mapName);
		}
		hPrintf("</TD></TR>");
	...
	case tvFull:
		if (!nextItemCompatible(track))
		{
			if (trackIsCompositeWithSubtracks(track))  //TODO: Change when tracks->subtracks are always set for composite
			{
					...
					if (isSubtrackVisible(subtrack))
					{
						...
						else
						{
							if (isCenterLabelIncluded(subtrack))
								y += fontHeight;
							//fprintf(stderr, "doTrackMap::case tvFull() for %s:%d\n", subtrack->shortLabel, subtrack->limitedVis == tvDense);
							if(theImgBox && subtrack->limitedVis == tvDense) {
								//fprintf(stderr, "doTrackMap::case tvFull() for %s\n", track->shortLabel);
				...
				// Hack: do stuff for compSeries tracks
				if(trackDbSetting(track->tdb, "compSeries") != NULL) {
					//fprintf(stderr, "doMapItems::case tvFull() for %s:%d:%d\n", track->shortLabel, y, track->lineHeight);
					mapBoxToggleVis(hvg, trackPastTabX, y, trackPastTabWidth, track->lineHeight, track);
				}
			...
			//fprintf(stderr, "trackPlusLabelHeight::case tvFull() for %s\n", track->shortLabel);
			y += trackPlusLabelHeight(track, fontHeight);
		}
		break;
	case tvDense:
		...
		//fprintf(stderr, "lbl: %s, lineHeight: %d, totalHeight: %d, fontHeight: %d\n", track->shortLabel,
		//	track->lineHeight, track->totalHeight(track, track->limitedVis), fontHeight);
		if (tdbIsComposite(track->tdb))
		...
		// ***** NOT SURE if this is the right thing to do, but appears to be working *****
		y += tdbIsComposite(track->tdb)? mapHeight: track->totalHeight(track, track->limitedVis);
		break;
	case tvHide:
	default:
		break;  /* Do nothing; */
...
{
	struct track *track;
	struct track *shadeTrack = NULL;
	int num_color = 16;
	Color shade_colors[16];
	MgFont *font = tl.font;
	struct hvGfx *hvg;
	struct tempName gifTn;
	char *mapName = "map";
...
#define maxSafeHeight   32000
	...
	for (track = trackList; track != NULL; track = track->next)
	{
		...
		if (!strcasecmp(track->shortLabel, "multishade")) {
			shadeTrack = track;
			//hPrintf("<!-- Test information: shadeTrack->items: %d -->\n", shadeTrack->items==NULL);
		}
		if (!safeHeight)
		{
	...
	if (withLeftLabels && psOutput == NULL)
	{
		...
		for (flatTrack = flatTracks; flatTrack != NULL; flatTrack = flatTrack->next)
		{
			...
			if (track->limitedVis != tvHide)
			{
				...
				// TODO: This function will be re-enabled, but with certain change
				if (track->hasUi)
				{
					if(tdbIsCompositeChild(track->tdb))
					{
						struct trackDb *parent = tdbGetComposite(track->tdb);
						//fprintf(stderr, "mapBoxTrackUi(name: %s; short label: %s; id: "
						//	"%s;) in makeActiveImage(isComposite)\n", 
						...
						//fprintf(stderr, "mapBoxTrackUi(name: %s; short label: %s; id: "
						//	"%s;) in makeActiveImage(notComposite)\n", 
						//	track->track, track->shortLabel, track->track);
	...
	/* Draw guidelines. */
	{
		...
		if(theImgBox)
		{
			...
			char base[64];
			safef(base,sizeof(base),"shade%d-%d-%s%d-%d",cart->sessionId, 
				pixWidth,(revCmplDisp?"r":""),insideX,guidelineSpacing);  // reusable file needs width, leftLabel start and guidelines
#ifdef USE_PNG
		...
		if (!exists)
		{
			...
			*/
			...
			if(CBIsInBrowser && shadeTrack != NULL) {
				...
				double scale = scaleForWindow(insideWidth, winStart, winEnd);
				struct slList *item;
				/* Hack: build an array of shade colors */
				shade_colors[0] = hvGfxFindColorIx(bgImg, 255, 223, 85);		// light yellow
				shade_colors[1] = hvGfxFindColorIx(bgImg, 170, 223, 255);		// light cyan
				shade_colors[2] = hvGfxFindColorIx(bgImg, 255, 159, 170);		// light pink
				shade_colors[3] = hvGfxFindColorIx(bgImg, 127, 255, 85);		// light green
				shade_colors[4] = hvGfxFindColorIx(bgImg, 127, 159, 255);		// light blue
				shade_colors[5] = hvGfxFindColorIx(bgImg, 255, 159, 85);		// light orange
				shade_colors[6] = hvGfxFindColorIx(bgImg, 170, 127, 255);		// light purple
				shade_colors[7] = hvGfxFindColorIx(bgImg, 200, 200, 200);		// light gray
				shade_colors[8] = hvGfxFindColorIx(bgImg, 255, 191, 0);			// yellow
				shade_colors[9] = hvGfxFindColorIx(bgImg, 42, 191, 255);		// cyan
				shade_colors[10] = hvGfxFindColorIx(bgImg, 230, 50, 50);		// red
				shade_colors[11] = hvGfxFindColorIx(bgImg, 42, 159, 0);			// green
				shade_colors[12] = hvGfxFindColorIx(bgImg, 42, 127, 170);		// blue
				shade_colors[13] = hvGfxFindColorIx(bgImg, 255, 102, 0);		// orange
				shade_colors[14] = hvGfxFindColorIx(bgImg, 170, 31, 255);		// purple
				shade_colors[15] = hvGfxFindColorIx(bgImg, 145, 145, 145);		// gray

				//hPrintf("<!-- Test information: shadeTrack->items: %d -->\n", shadeTrack->items==NULL);
				for (item = shadeTrack->items; item != NULL; item = item->next) {
					void *item_trans = item;
					struct bed *bed = item_trans;
					int x1 = round((double)((int)bed->chromStart-winStart)*scale) + insideX;
					int x2 = round((double)((int)bed->chromEnd-winStart)*scale) + insideX;
					//x1=x1+x2+color;
					//hPrintf("<!-- Test information: x1: %d; x2: %d, score: %d -->\n", x1, x2, bed->score % num_color);
					hvGfxBox(bgImg, x1, 0, x2-x1, pixHeight, shade_colors[bed->score % num_color]);
				}
	...
	/* Draw center labels. */
	if (withCenterLabels)		// if the environment is within CEpBrowser, no center labels
	{
	...
	/* Draw tracks. */
	{
		...
		for (flatTrack = flatTracks; flatTrack != NULL; flatTrack = flatTrack->next)
		{
			...
			//fprintf(stderr, "track: %s; yStart: %d; yEnd: %d\n", track->track, yStart, yEnd);
			if(theImgBox)
			{
				...
				if(sliceHeight > 0)
				{
					//fprintf(stderr, "imgTrackSliceUpdateOrAdd(%d, %d, %d, %d)\n", sliceWidth[stData],sliceHeight,sliceOffsetX[stData],sliceOffsetY);
			...
			if (theImgBox && track->limitedVis == tvDense && tdbIsCompositeChild(track->tdb)) {
				//fprintf(stderr, "<!-- function: makeActiveImage() for %s -->\n", track->shortLabel);
	...
	*		will be disabled for speed. */
{
	...
	// Change: it depends whether this hgTracks is a private one.
	//			If it is a private one and is in a comparative epigenomic browser,
	//				then CBIsInBrowser = TRUE and dragZooming = FALSE
#ifndef PRIVATE_CGI
	...
	if (showTrackControls && !CBIsInBrowser) {
		hButton("hgt.refresh", "refresh");
	}
	...
	if (CBIsInBrowser) {
		...
		for (group = groupList; group != NULL; group = group->next)
		{
			if (group->trackList == NULL)
				continue;
			if (differentString(group->name, "lab")) {
				// Only lab specific tracks needs to be done here
				continue;
			}
			...
			hPrintf("</DIV><DIV STYLE=\"display:none\" id=\"TrackUnique\">\n");
