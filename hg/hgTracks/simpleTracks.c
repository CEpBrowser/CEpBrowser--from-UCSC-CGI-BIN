...
/* Hack begins
 * Added shadesOfTransparent
 */
#ifdef COLOR32
int maxTransparencyShade = 49;
Color shadesOfTransparency[50+1];
#else
int maxTransparencyShade = 9;
#endif
/* Hack ends */
...
if (x < xEnd)
    ...
    if(theImgBox && curImgTrack)
        ...
        #endif//def IMAGEv2_SHORT_MAPITEMS
			//fprintf(stderr, "mapBoxHgc():%s", track);
        imgTrackAddMapItem(curImgTrack,link,(char *)(statusLine!=NULL?statusLine:NULL),x, y, xEnd, yEnd, track);
        }
    else
        {
        hPrintf("<AREA SHAPE=RECT TARGET=\"_blank\" COORDS=\"%d,%d,%d,%d\" ", x, y, xEnd, yEnd);
        if (directUrl)
            {
            hPrintf("HREF=\"");
...
// Hack: make shadeOfTransparency
#ifdef COLOR32
void makeTransparency(struct hvGfx *hvg)
/* Make eight shades of gray in display. */
{
hMakeTransparency(hvg, shadesOfTransparency, maxTransparencyShade);
shadesOfTransparency[maxTransparencyShade+1] = MG_RED;
}
#endif
// End hack
...
{
makeGrayShades(hvg);
// Hacked: add transparency
#ifdef COLOR32
makeTransparency(hvg);
#endif
// End hack
makeBrownShades(hvg);
makeSeaShades(hvg);
...
// Hack: Added transparentInRange for better granularity
int transparentInRange(int val, int minVal, int maxVal)
{
	return hGrayInRange(val, minVal, maxVal, maxTransparencyShade);
}
...
// Don't bother if we are imageV2 and a dense child.
	// also do not draw this in comparative browser
if(!CBIsInBrowser && (!theImgBox || tg->limitedVis != tvDense || !tdbIsCompositeChild(tg->tdb)))
    {
    char *directUrl = trackDbSetting(tg->tdb, "directUrl");
    boolean withHgsid = (trackDbSetting(tg->tdb, "hgsid") != NULL);
 //   fprintf(stderr, "mapBoxHgcOrHgGene(id: %s;) in genericMapItem\n", 
	//tg->track);
