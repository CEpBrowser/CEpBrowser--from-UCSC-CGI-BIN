...
for(slice = imgTrack->slices;slice != NULL;slice=slice->next)
    ...
    && bottomRightY > (slice->offsetY + 1)) // Overlap of a pixel or 2 is tolerated
        ...
        if(map!=NULL)
			...
			//fprintf(stderr, "mapOffset id=%s, (%d,%d,%d,%d), (%d,%d,%d,%d)\n", id,
			//	topLeftX, topLeftY, bottomRightX, bottomRightY, slice->offsetX, slice->offsetY,
			//	slice->offsetX + slice->width, slice->offsetY + slice->height);
...
for(;item!=NULL;item=item->next)
    ...
    if(map->linkRoot != NULL)
        {
#ifndef PRIVATE_CGI
		if((strncmp(map->linkRoot, CEPBROWSER_HGTRACKS, strlen(CEPBROWSER_HGTRACKS))) 
			|| !strncmp(map->linkRoot, CEPBROWSER_HGTRACKUI, strlen(CEPBROWSER_HGTRACKUI))) {
				// not hgTracks, so need to specify target
				hPrintf(" TARGET=\""CEPBROWSER_TARGET"\"");
		}
#endif
        ...
        {
#ifndef PRIVATE_CGI
		if((strncmp(item->linkVar, CEPBROWSER_HGTRACKS, strlen(CEPBROWSER_HGTRACKS))) 
			|| !strncmp(item->linkVar, CEPBROWSER_HGTRACKUI, strlen(CEPBROWSER_HGTRACKUI))) {
				hPrintf(" TARGET=\""CEPBROWSER_TARGET"\"");
		}
#endif
        if(skipToSpaces(item->linkVar))
...
#else
		hPrintf("  <A TARGET=\""CEPBROWSER_TARGET"\" HREF=%s",slice->link);
#endif
    else
#ifdef PRIVATE_CGI
        hPrintf("  <A HREF='%s'",slice->link);
